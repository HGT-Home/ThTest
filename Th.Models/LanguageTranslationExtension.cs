...
namespace Th.Models
{
    ...
    {
        public static string GetLanguageText<T>(this ILanguageTranslation<T> translation, string strLang = "", [CallerMemberName]string strPropertyName = null) where T:ITranslation
        {
            if(translation != null && translation.Translations != null && translation.Translations.Count > 0)
            {
                if (string.IsNullOrEmpty(strLang))
                {
                    strLang = Thread.CurrentThread.CurrentCulture.Name;
                }
